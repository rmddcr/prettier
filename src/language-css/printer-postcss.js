...
const insideInSCSSMapNode = utils.insideInSCSSMapNode;
const insideInNestedSCSSMapNode = utils.insideInNestedSCSSMapNode;
...
function genericPrint(path, options, print) {
  ...
  switch (node.type) {
    ...
    case "value-paren_group": {
      ...
      const insideInSCSSMap = insideInSCSSMapNode(path);
        ...
        concat([
          ...
          indent(
            concat([
              softline,
              join(
                concat([",", insideInSCSSMap ? hardline : line]),
                path.map(childPath => {
                  const printed = print(childPath);

                  if (insideInSCSSMap && insideInNestedSCSSMapNode(path)) {
